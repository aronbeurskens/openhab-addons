...
The PLEX Server needs to be configured first. The hostname of the PLEX server is mandatory and the either the PLEX token (recommended) or the username/password of the PLEX server.

The find the PLEX token please follow the instructions from the PLEX support forum: 
...
Turn on the player you want to add and *play* some media on it. Navigate to `/settings/things/add/plex` and press the `SCAN` button.
The player will then be autodiscovered. 
...
|----------------------|--------|------------|----------------------------------------------------------------|
| currentPlayers       | String | RO         | The count of players currently configured to watch on PLEX     |
| currentPlayersActive | String | RO         | The count of players currently being used on PLEX              |
| state                | String | RO         | The current state of the Player                                |
| power                | String | RO         | The power status of the player                                 |
| title                | String | RO         | The title of media that is playing                             |
| type                 | String | RO         | The current type of playing media                              |
| endtime              | String | RO         | Time at which the media that is playing will end               |
...
| player               | String | RW         | The control channel for the player `PLAY/PAUSE/NEXT/PREVIOUS`  |
...
You can also design some rules from the openHAB UI:

![Dim lights when playing movie](doc/Rules_DimLightsWhenPlayingMovie.png)
![Adjust lights when movie is paused](doc/Rule_AdjustLightWhenMovieIsPaused.png)
![Enable lights when movie is stopped](doc/Rule_EnableLightsWhenPlayerIsStopped.png)
